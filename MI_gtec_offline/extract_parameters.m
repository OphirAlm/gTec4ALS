function [Hz, chunckSz, nClass, subID, f, numTrials, RS_len] = extract_parameters(S)

posHz       = S.Hz.Value;                       % Get sampling rate value
Hz          = str2double(S.Hz.String(posHz,:));

chunckSz    = str2double(S.ChunkSz.String);       % Get Buffer size (in sec) times sampling rate
